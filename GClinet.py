...
class client:
    ...
    def Log_check(self):
        ...
        try :self.tcpCliSock.connect(self.ADDR)
        except: m=messagebox.askokcancel(title = '提示',message='连接失败，请检查你的网络')
        else:
            self.Login()
    def Login(self):
        ...
        bt = Button(self.fm1, text='确定', command=lambda:self.View(name_E.get(),ps__E.get()))
    ...
    def Regist(self):
        ...
        bt1 = Button(self.fm2, text='确定', command=lambda:self.Regist_check(name_E.get(),stu_E.get(),ps_E.get())).place(x=650, y=400)
    ...
    def View(self,name_g,ps_g):
        ...
        def addjob():
            ...
            addB=Button(add,text='确定',command=lambda:adding({'name':name.get(),'sex': sex.get(), 'student_number':stu.get(), 'group_number':gro.get(),
